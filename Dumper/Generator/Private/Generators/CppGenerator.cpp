...
void CppGenerator::GenerateVSProject(const std::vector<std::string>& AllHppFiles, const std::vector<std::string>& AllCppFiles)
{
	...
	const std::string ProjectGuid = GenerateGuid("Dumper7-" + RawGameName);

	// ── .slnx ──
	{
		StreamType Slnx(MainFolder / (GameName + ".slnx"));
		Slnx << R"(<Solution>
  <Project Path=")" << GameName << R"(.vcxproj" />
</Solution>
)";
	}
	...
	// ── Main.cpp ──
	{
		StreamType MainCpp(MainFolder / "Main.cpp");
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD reason, LPVOID lpReserved)
{
	...
	// ── .vcxproj ──
	{
		StreamType Vcxproj(MainFolder / (GameName + ".vcxproj"));
...
<Project DefaultTargets="Build" xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  ...
  <PropertyGroup Label="Globals">
    <ProjectGuid>)" << ProjectGuid << R"(</ProjectGuid>
  ...
  <Import Project="$(VCTargetsPath)\Microsoft.Cpp.Default.props" />
		...
		// Property groups for both configurations
  ...
  <ItemDefinitionGroup Condition="'$(Configuration)|$(Platform)'=='Debug|x64'">
    <ClCompile>
      ...
      <PreprocessorDefinitions>_DEBUG;%(PreprocessorDefinitions)</PreprocessorDefinitions>
		...
		// ClCompile items — Main.cpp, Basic.cpp, CoreUObject_functions.cpp always active
		// Other _functions.cpp are commented out; uncomment as needed (see UsingTheSDK.md)
		Vcxproj << "  <ItemGroup>\n";
